...
def setup_scheduler() -> AsyncIOScheduler:
    ...
    # 5:10 JST - First data fetch (after US market close at ~5:00 JST / 16:00 ET)
    scheduler.add_job(
        _run_daily_fetch,
        CronTrigger(hour=5, minute=10, timezone="Asia/Tokyo"),
        id="daily_fetch_1",
        name="Daily data fetch (5:10)",
    )

    # 5:25 JST - Retry
    scheduler.add_job(
        _run_daily_fetch,
        CronTrigger(hour=5, minute=25, timezone="Asia/Tokyo"),
        id="daily_fetch_2",
        name="Daily data fetch retry (5:25)",
    )

    # 9:05 JST - Morning actual open prices
    scheduler.add_job(
        _run_morning_fetch,
