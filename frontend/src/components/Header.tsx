...
export default function Header({ lastUpdated, activeTab, onTabChange }: Props) {
  return (
    <header className="sticky top-0 z-50 bg-white/92 backdrop-blur-[12px] border-b border-border h-14 px-8 flex items-center justify-between">
      <button
        ...
        className="flex items-center gap-2.5 font-semibold text-[17px] tracking-tight cursor-pointer hover:opacity-75 transition-opacity"
      >
      ...
      <div className="flex items-center gap-3">
        ...
        {([['pair-research', 'ペア調査'], ['ai-analyst', 'AIアナリスト'], ['settings', '⚙ 設定']] as const).map(([id, label]) => (
          <button
            key={id}
            onClick={() => onTabChange(id)}
            className={`text-[12.5px] font-medium px-3 py-1.5 rounded-md transition-all cursor-pointer ${
              activeTab === id
